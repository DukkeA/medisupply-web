...
import { useWarehouses, useCreateWarehouse } from './use-warehouses'
import {
...
vi.mock('@/generated/api', () => ({
  WebApi: vi.fn().mockImplementation(() => ({
    getWarehousesBffWebWarehousesGet: vi.fn().mockResolvedValue({
      data: {
        items: [],
        total: 0,
        page: 1,
    ...
    createWarehouseBffWebWarehousePost: vi.fn().mockResolvedValue({
      data: {
        id: 1,
        name: 'Test Warehouse',
...
describe('useWarehouses', () => {
  ...
  it('should fetch warehouses successfully', async () => {
    const { result } = renderHookWithProviders(() => useWarehouses(10, 0))
  ...
  it('should pass correct limit and offset to query', async () => {
    const { result } = renderHookWithProviders(() => useWarehouses(20, 5))
  ...
  it('should use mock data when NEXT_PUBLIC_MOCK_DATA is true', () => {
    ...
    const mockData = {
      items: [{ id: 1, name: 'Test Warehouse' }],
      total: 1,
      page: 1,
      size: 10,
      has_next: false,
...
describe('useCreateWarehouse', () => {
  ...
  it('should create warehouse mutation', () => {
    const { result } = renderHookWithProviders(() => useCreateWarehouse())
  ...
  it('should invalidate warehouses query on success with custom QueryClient', async () => {
